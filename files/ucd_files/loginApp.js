...
//Below method will call duo method in Duo-Web-v2.js with duoSecurityChallenge
//after completion duo will call "duoSecurityCallback" method to provide response
...
//this function will be called from Duo-Web-v2.js after completing duo authn process

  var duoSecurityCallback = function(details, payload) {
    var credentials = {};
...
this.removeBtnAndShowSpinner = function(btn) {
    ...
    this.pushPollInterval = setInterval(function() {
      ...
      var signinDiv = document.getElementById("signin-div");
        if ( ( signinDiv ) &&
          ...
          var payload = self.buildPayload("submitPushPoll", signinDiv);
  ...
  this.showResendCodeOption = function(formDiv, obj, timeInMilis) {
    ...
    if (formDiv && (formDiv.whichFactorForm === "EMAIL" || formDiv.whichFactorForm === "SMS" ) && formDiv.noResend == null) {
      ...
      var didNotGetMsg = this.localizeMsg(formDiv.whichFactorForm.toLowerCase() + '-did-not-get-msg', 'Did not get the message?');
      var resendMsg = this.localizeMsg(formDiv.whichFactorForm.toLowerCase() + '-resend-btn', 'Resend message');
      ...
      setTimeout(function() {
        ...
        resendDivElem.className += " sameline";
        ...
        var submitBtnElem = formDiv.querySelector("#submit-btn");
        if (submitBtnElem) {
  ...
  this.showResendEmail = function(formDiv, obj, timeInMilis) {
    if (formDiv && formDiv.whichForm != null && formDiv.whichForm === "FORGOT_PASSWORD_FORM") {
      ...
      var didNotGetMsg = this.localizeMsg('forgot-pw-did-not-get-msg','Did not get the email?');
      var resendMsg = this.localizeMsg('forgot-pw-resend-btn', 'Resend email');
  ...
  this.showReinputUserName = function(formDiv, obj, timeInMilis) {
    if (formDiv && formDiv.whichForm != null && formDiv.whichForm === "FORGOT_PASSWORD_FORM") {
      ...
      var didNotGetMsg = this.localizeMsg('forgot-pw-incorrect-username-msg','Incorrect UserName?');
      var resendMsg = this.localizeMsg('forgot-pw-incorrect-username-btn','Fix UserName');
  ...
  this.mask = function(msg) {
    var propsToMask = ['username','password','bypasscode','otpcode','questions','deviceid','requeststate','phonenumber','token','authntoken','trusttoken','userid'];
  ...
  this.replaceDiv = function(divid,replacement,dofocus) {
      ...
      if ( dofocus ) {
        ...
        if ( div ) {
          var firstInput = div.querySelector('input[type="text"]');
  ...
  this.validateForm = function(formDiv) {
    ...
    var inputFields = formDiv.getElementsByTagName("INPUT");
    var isError = false;
    for (i=0; i < inputFields.length; i++) {
      ...
      if (inputFields[i].required && inputFields[i].value.trim() === '') {
        ...
        inputFields[i].className += " on__error";
      }
    }
    if (isError) {
      var errorMessage = this.localizeMsg('error-required-fld','Required field empty');
  ...
  this.handleFocusOutEvent = function(elem) {
    elem.addEventListener('focusout', function() {
      if (elem.value.trim().length == 0) {
        ...
        elem.className += " on__error";
      }
      else {
        //elem.classList.remove('on__error');
        elem.className.replace(' on__error', ' ')
      }
    });
  };
  ...
  this.handleClickToSubmitEvent = function(formDiv, obj, methodName, includeAuthnFactor) {
    ...
    formDiv.querySelector("#submit-btn").onclick = function() {
      if (obj.validateForm(formDiv)) {
        var payload = obj.buildPayload(methodName, formDiv);
  ...
  this.buildPayload = function(methodName, formDiv) {
    ...
    var preferredFactorElem = document.getElementById("preferredFactorOption");
    ...
    switch (methodName) {
      ...
      case 'postSecQuestions':
        var questions = [];
        var inputElements = formDiv.getElementsByTagName("INPUT");
  ...
  this.handleOptionalMFAEnrollment = function(payload, formDiv, obj) {
    ...
    if (payload.mfaSettings) {
      ...
      if (!payload.mfaSettings.enrollmentRequired) {
        ...
        skipEnrollDivElem.className += " sameline";
  ...
  this.showSwitchEnrollFactorOption = function(formDiv, obj) {
    ...
    if (formDiv && (formDiv.step === "enrollment")) {
      ...
      var enrollSwitchMsg = this.localizeMsg('enroll-switch-msg','Switch Factor to Enroll');
      var orMsg = this.localizeMsg('or-msg','OR');

      var switchEnrollDivElem = document.createElement('div');
      //switchEnrollDivElem.classList.add('newline');
      switchEnrollDivElem.className += " newline";
...
/* ----------------------------------------------------------------------------------------------------------------
      ...
      if ( step === "submitCreds" ) {
        ...
        if (which !== "USERNAME_PASSWORD") {
          ...
          if (payload.nextOp.indexOf("getBackupFactors") > 0) {
            ...
            var altFactorsMsg = this.localizeMsg('backup-btn', 'Use an Alternative Factor');

            var altFactorsDivElem = document.createElement('div');
            //altFactorsDivElem.classList.add('sameline');
            altFactorsDivElem.className += " sameline";
  ...
  this.buildFirstForm = function(formDiv,showUidPw,IDPdata) {
    ...
    /*
        var message = '<div class="row">'+
          '<div class="col-md-12">' +
          '<div class="alert alert-warning" role="alert"><span><p>Scheduled System Maintenance: <br><br/>The UCD Access Portal will be unavailable from 6 pm, Saturday, September 21, until 11:59 pm, on Sunday, September 22. For more information, please refer to the Service Alerts tab on the University Information Systems (UIS) homepage at <a href="http://www.cu.edu/uis">www.cu.edu/uis</a>. <br><br/>Please note: Most university applications like Canvas, E-mail, MyAccount will be unaffected by this outage. </p></span></div>                                    </div>'+
          '</div>';
    */
    ...
    if(xhr.addEventListener) {
      ...
      xhr.addEventListener("readystatechange", function () {
        ...
        if (this.readyState === 4) {
          ...
          var insertMessageBox = '';

          if (!(message == null || message.trim().length === 0)) {
            insertMessageBox = "<div class=\"row\"><div class=\"col-md-12\"><div class=\"alert alert-warning\" role=\"alert\"><span>"
              + message
              + "</span></div></div></div>";
          }
          ...
          if (showUidPw) {
            formDiv.innerHTML += '<div class="login-clean">' +
              '<form class="form-horizontal" role="form" id="FormValues" autocomplete="off">' +
              '<div class="row">' +
              '<div class="col-md-12"><img class="img-responsive" src="../images/dualCampus_sm_240.png" alt="CU Denver | Anschutz Medical Campus">' +
              '<hr>' +
              '</div>' +
              '</div>' +
              insertMessageBox +
              ...
              '<div class="col-md-12">' +
              '<h1>Web login </h1>' +
              '<p class="lead text-muted">Please sign in with your university credentials.</p>' +
              '</div>' +
              '</div>' +
              '<div class="form-group"><label for="username" class="control-label">Username or email</label><input class="form-control" type="text" id="userid" value="" required name="username">    <div class="form-group"><label for="password" class="control-label">Password </label><input class="form-control" type="password" id="password" value="" required name="password"></label></div>' +
              '<div class="form-group"><label class="error-msg" id="login-error-msg"></label><button type="button" class="btn btn-primary btn-block submit" id="submit-btn" data-res="signin-submit-btn">Sign In</button></div><a href="http://www.ucdenver.edu/forgotpassword" class="forgot">Forgot your password?</a><a href="https://www1.ucdenver.edu/offices/office-of-information-technology/get-help" class="forgot">Need help?</a><a href="http://www.ucdenver.edu/policy/Pages/PrivacyPolicy.aspx" target="_blank" title="Privacy Policy" class="forgot">Privacy Policy</a></form>' +
              '</div>';
          }
          ...
          if (showUidPw) {
            self.handleClickToSubmitEvent(formDiv, self, 'postCreds');
            self.handleKeyPressToSubmitEvent(formDiv, formDiv.querySelector("#password"), self, 'postCreds');
          }
          return formDiv;
      ...
      xhr.open('GET', '/warningMessage/', true);
      xhr.send(null);
    } else {
        // then show the UID + PW form if needed
        if (showUidPw) {
  ...
  // Remove Form Elements for DUO MultiFactor
  this.updateFormForDuo = function() {

    var formDiv = document.createElement('div');
    formDiv.id = 'FormValues';
    formDiv.innerHTML = '<iframe id="duo_iframe"></iframe>';

    this.replaceDiv("FormValues",formDiv,true);

  }; // this.updateFormForDuo
  ...
  this.displayEnrollmentOptionsForm = function(payload) {
    ...
    for (i=0; i < payload.nextAuthFactors.length; i++) {
      var factor = payload.nextAuthFactors[i];
  ...
  this.displaySocialRegistrationForm = function(socialData) {
    ...
    formDiv.className += " sign-in";
      ...
      formDiv.querySelector("#social-submit-btn").onclick = function() {
          ...
          data.callbackUrl = "http://www.google.com";
      ...
      formDiv.querySelector("#social-cancel-btn").onclick = function() {
          ...
          window.location.href='./signin_orig.html';
      };
  ...
  this.displayAltFactorsSubform = function(payload) {
    var self = this;
    var otherFactorsDiv = document.createElement('div');
    // otherFactorsDiv.classList.add("alt-factor-pane");
    otherFactorsDiv.className += " alt-factor-pane";
    ...
    var currentFactor = document.getElementById("signin-div").whichFactorForm;
    // which device displayName is currently being used?
    var displayNameOnScreen = document.getElementById("signin-div").whichDisplayName;
...
//                '<span class="tooltiptext" data-res="factor-' + factor.toLowerCase() + '-desc">' + self.AuthenticationFactorInfo[factor].description + '</span>';
              var currentDevice = payload[factor].enrolledDevices[i];
  ...
  this.displayForgotPassWordForm = function(payload) {
    ...
    backToLoginDivElem.className += " newline";
    ...
    if (payload.requestState && payload.nextOp) {
      ...
      if (payload.nextOp[0] === 'credSubmit') {
        if(payload.nextAuthFactors) {
          ...
          var duoSecurityAuthFactor = false;

          payload.nextAuthFactors.forEach(function(factor) {
            if (payload[factor] && payload[factor].enrolledDevices && payload[factor].enrolledDevices.length > 0) {
              sameFactorMultipleDevices = true;
            }
            // Set the DUO flag if we receive a DUO Security Auth Factor
            if (payload[factor] === "DUO_SECURITY") {
              duoSecurityAuthFactor = true;
            }
          });
          ...
          // If DUO AuthFactor: Pass data to iframe for Duo
          else if(duoSecurityAuthFactor || payload.DUO_SECURITY) {
            this.updateFormForDuo();
            duo(payload, duoSecurityCallback);
          } else {
            // ER #1
        ...
        else if (payload.nextOp.indexOf('enrollment') == -1) { // Alternative factors case
          var which = Object.keys(self.AuthenticationFactorInfo).filter( function(x) { return x in payload;});
          this.logMsg('nextOperation: Factor is ' + which[0]);
          if(payload.DUO_SECURITY) {
            // Pass data to iframe for Duo
            this.updateFormForDuo();
            duo(payload, duoSecurityCallback);
          } else if (typeof which[0] === 'undefined') { // PUSH alternative factor case, when there's no 'PUSH' in the payload.
        ...
        else if(payload.DUO_SECURITY) {
          // Pass data to iframe for Duo
            this.updateFormForDuo();
            duo(payload, duoSecurityCallback);
        }
        else if (payload.EMAIL || payload.SECURITY_QUESTIONS || payload.PUSH || payload.TOTP) {
      ...
      // Added the check payload.nextOp.indexOf('createToken') >= 0 below.
        ...
        // Pass data to iframe for Duo
        else if(payload.nextAuthFactors && payload.nextAuthFactors[0] === "DUO_SECURITY" && payload.mfaSettings && payload.mfaSettings.enrollmentRequired && payload.mfaSettings.enrollmentRequired === true) {
          self.sdk.postDuoEnrollment(payload);
        }
        else {
          this.displayEnrollmentOptionsForm(payload);
        }
